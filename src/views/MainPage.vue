  ...
  <div class="container">
    ...
    <div class="main-content">
      <v-chip v-for="(terminal, index) in terminals" :key="terminal.id" @contextmenu.prevent="openContextMenu(index)"
        label closable @click:close="closeTerminal(index, terminal.id)" @click="changeTerminal(index,terminal.id)" style="margin-left:5px">{{
...
export default {
  ...
  setup() {
    ...
    const doConnect = (link) => {
        ...
        store.commit('setActiveTerminalIndex', terminals.value.length - 1);
        //跳转路由
        router.push({ name: 'terminal_index'});
